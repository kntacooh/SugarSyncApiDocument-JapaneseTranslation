...
3. Create your app using the [*:::Developer Console:::*](https://www.sugarsync.com/developer/account). This identifies the app to SugarSync. After your app is identified in this way to SugarSync, you can develop it. See [Managing Your Access Keys and Applications](/source/dev/managing-apps.html) for further details.
4. Read the remainder of this Getting Started guide. Learn about the [resources available through the API](/source/dev/resources.html) and examine the [code examples](/source/dev/using-api.html). Then browse through our [Resource Reference](/source/dev/api/resource-ref.html) for details on the syntax and semantics of Platform API requests, and start developing.

**[Can't wait? Get started with our Sample App](/source/dev/using-api.html)**
...
* Clients interact with servers by requesting resources. A resource is any meaningful concept that can be addressed. In the context of the Platform API, this means that:
  ...
  * Your application makes requests through the API for resources such as users, folders, files, and collections (that is, groups of objects).
  ...
  See the [Resource Overview](/source/dev/resources.html) for further information about SugarSync resources that you can access through the Platform API.
...
Note that the URL of the user resource in the HTTP request example is only illustrative. To obtain the actual URL, your application needs to obtain a refresh token, which it then uses to get an access token. See [Authorization to Make API Requests](#authorization-to-make-api-requests) for more details. In the process of generating an access token, the SugarSync service returns the URL of the user resource.
...
The XML data that follows the header is the response body, which contains the requested resource. SugarSync represents resources as XML, as you'll see in [The Platform API and XML](/source/dev/getting-started.html#the-platform-api-and-xml).
...
For code examples, see [Using the Platform API: Examples](/source/dev/using-api.html). Also see the [Resource Reference](/source/dev/api/resource-ref.html) for details on the syntax and semantics of Platform API requests. Good luck with your project!
